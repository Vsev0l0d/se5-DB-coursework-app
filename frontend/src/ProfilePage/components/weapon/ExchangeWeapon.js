...
export const ExchangeWeapon = (props) => {
    useEffect(() => {
        M.AutoInit()
    }, [])
    ...
    const handleSubmit = (event) => {
        ...
        if (id != JSON.parse(localStorage.getItem("id"))) {
            dispatch(personageModel.thunks.exchangeWeapon(props.weaponLink, id))
        }
    }

    const forHref = `#exchangeWeapon${props.id}`
    const forId = `exchangeWeapon${props.id}`
    ...
    return <>
        ...
        <div id={forId} className="modal">
            <div className="modal-content">
                <div className="row">
                    <span className="col s12">Подарить оружие</span>
